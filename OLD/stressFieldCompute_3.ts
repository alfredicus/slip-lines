...
/**
 * Generate special equipotential curve at σ_n = σ₂ (meridian)
 */
export function generateSpecialEquipotential(
    lambda_x: number,
    lambda_y: number,
    lambda_z: number,
    config?: Partial<ComputeConfig>
    ...
    // Generate meridian at σ_n = σ₂
    for (let i = 0; i < cfg.curveResolution; i++) {
        const t = i / (cfg.curveResolution - 1);
        // Parametric meridian
        const l = Math.sin(t * Math.PI);
        const m = 0;
        const n = Math.cos(t * Math.PI);
